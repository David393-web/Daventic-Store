...
import { Loader2, Eye, EyeOff, ChevronDown } from 'lucide-react'; // 👈 Added ChevronDown
...
const SellerSignupPage = () => {
  ...
  const sellerSignUp = async (data) => {
    try {
      ...
      console.error("Seller Signup API Error:", error.message);
      throw error;
    }
  };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // ✅ set craftCategories from dropdown
      data.craftCategories = selectedCategories;

      const response = await sellerSignUp(data);

      toast.success(response.message || "Seller signup successful!", {
        position: "top-right",
        autoClose: 1000,
        onClose: () =>
  ...
  return (
    <div className="flex w-screen h-screen font-sans text-gray-800">
      ...
      {/* Right Side - Seller Sign Up Form */}
      <div className="flex items-center justify-center w-full p-8 md:w-1/2 bg-gray-50 sm:p-12">
        <div className="w-full max-w-md">
          <h2 className="mb-8 text-3xl font-bold text-center">Seller Sign Up</h2>
            ...
            <div>
              ...
              <div className="relative mt-1">
                <button
                  ...
                  className="w-full flex justify-between items-center py-2 px-3 border-b border-gray-300 bg-transparent focus:outline-none focus:border-blue-500"
                >
                ...
                {open && (
                  <ul className="absolute z-10 w-full bottom-full mb-2 bg-white border rounded shadow-lg max-h-48 overflow-y-auto">
                    <li
                      className={`px-4 py-2 cursor-pointer hover:bg-blue-100 ${selectedCategories.includes("Fashion") ? "bg-blue-50 font-semibold" : ""
                        }`}
                    ...
                    <li
                      className={`px-4 py-2 cursor-pointer hover:bg-blue-100 ${selectedCategories.includes("Jewelry") ? "bg-blue-50 font-semibold" : ""
                        }`}
                    ...
                    <li
                      className={`px-4 py-2 cursor-pointer hover:bg-blue-100 ${selectedCategories.includes("Home Decor") ? "bg-blue-50 font-semibold" : ""
                        }`} 
                      onClick={() => toggleCategory("Home Decor")}
                    >
                      Home Decor
                    </li> 
                    <li
                      className={`px-4 py-2 cursor-pointer hover:bg-blue-100 ${selectedCategories.includes("Art & collectibles") ? "bg-blue-50 font-semibold" : ""
                        }`} 
                      onClick={() => toggleCategory("Art & Collectibles")}
                    >
                      Art & Collectibles
                    </li> 
                    <li
                      className={`px-4 py-2 cursor-pointer hover:bg-blue-100 ${selectedCategories.includes("Craft Supplies & Tools") ? "bg-blue-50 font-semibold" : ""
                        }`} 
                      onClick={() => toggleCategory("Craft Supplies & Tools")}
                    >
              ...
              {errors.craftCategories && (
                <p className="mt-2 text-sm text-red-600">{errors.craftCategories.message}</p>
              )}
            </div>
          ...
          <p className="mt-4 text-sm text-center text-gray-500">
            ...
            <Link to="/seller-login" className="text-blue-600 hover:underline">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
